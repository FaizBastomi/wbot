        ...
        try {
            ...
            if (availableLang.includes(args[0])) {
                ...
                for (let i = 0; i < searchResult.length; i++) {
                    textMsg += `*Pertanyaan*: ${searchResult[i].question.trim()}\n*Jawaban*: ${searchResult[i].answer.trim()}`
                        + `\n${searchResult[i].ansMedia ? "*Attachment*:\n" + searchResult[i].ansMedia.join("\n") + "\n\n" : "\n\n"}`
                }
                await msg.reply(textMsg);
            } else {
                searchResult = await search(args.join(" "));
                for (let i = 0; i < searchResult.length; i++) {
                    textMsg += `*Pertanyaan*: ${searchResult[i].question}\n*Jawaban*: ${searchResult[i].answer}`
                        + `\n${searchResult[i].ansMedia ? "*Attachment*:\n" + searchResult[i].ansMedia.join("\n") + "\n\n" : "\n\n"}`
                }
                await msg.reply(textMsg);
            }
...
const search = (question, lang = "id") => new Bluebird(async (resolve, reject) => {
