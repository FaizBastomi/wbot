...
const getJadwal = async (kotaId, tipe = "harian") => {
    try {
        let date = new Date(), config = { responseType: "json" }, resJadwalKota, teks = "";
        switch (tipe) {
            case "harian":
                resJadwalKota = await axios.get(`https://api.myquran.com/v1/sholat/jadwal/${kotaId}/${date.getFullYear()}/${date.getMonth() + 1}/${date.getDate()}`, config);
                teks += `Lokasi: ${resJadwalKota?.data?.data?.lokasi}\nDaerah: ${resJadwalKota?.data?.data?.daerah}\n\n`;
                teks += Object.keys(resJadwalKota?.data?.data?.jadwal).map(vue => `*${vue}*: ${resJadwalKota?.data?.data?.jadwal[vue]}`).join("\n");
                break;
            case "bulanan":
                resJadwalKota = await axios.get(`https://api.myquran.com/v1/sholat/jadwal/${kotaId}/${date.getFullYear()}/${date.getMonth() + 1}`, config);
                teks += `Lokasi: ${resJadwalKota?.data?.data?.lokasi}\nDaerah: ${resJadwalKota?.data?.data?.daerah}\n\n`;
                for (let idx in resJadwalKota?.data?.data?.jadwal) {
                    teks += Object.keys(resJadwalKota?.data?.data?.jadwal[idx])
                        .map(vue => `*${vue}*: ${resJadwalKota?.data?.data?.jadwal[idx][vue]}`).join("\n") + "\n\n";
                }
                break;
        }
        return teks;
