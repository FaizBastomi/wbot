...
const serialize = (msg, sock) => {
    if (msg?.message?.protocolMessage || msg?.message?.senderKeyDistributionMessage) {
        ...
        if (msg.key) {
            ...
            msg.sender =
                msg.isSelf ? (sock.user.id.split(":")[0] + "@s.whatsapp.net" || sock.user.id) : ((msg.key.participant.split(":")[0] + "@s.whatsapp.net" || msg.key.participant) || (msg.key.remoteJid.split(":")[0] + "@s.whatsapp.net" || msg.key.remoteJid));
