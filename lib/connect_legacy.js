const { makeWALegacySocket: WASocket, useSingleFileLegacyAuthState, DisconnectReason } = require("@adiwajshing/baileys");
...
const { legacySession } = require("../config.json");
const chatHandler = require("../event/chat_event");
const joinHandler = require("../event/group_event");

const { state, saveState } = useSingleFileLegacyAuthState(path(__dirname, `../${legacySession}`));
...
const connect = () => {
    ...
    sock.ev.on("connection.update", async (up) => {
        ...
        if (connection === "close") {
            if (reason === DisconnectReason.badSession) { console.log(`Bad Session File, Please Delete ${legacySession} and Scan Again`); process.exit(); }
            ...
            else if (reason === DisconnectReason.loggedOut) { console.log(`Device Logged Out, Please Delete ${legacySession} and Scan Again.`); process.exit(); }
